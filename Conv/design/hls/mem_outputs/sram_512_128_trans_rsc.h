...
template < 
  ...
  ,int ADDR_WIDTH
>
class sram_512_128_trans_rsc : public mc_wire_trans_rsc_base<128,512>
{
public:
  sc_in< bool >   clk0;
  sc_in< sc_logic >   csb0;
  ...
  typedef mc_wire_trans_rsc_base<128,512> base;
...
private:
  ...
  int get_addr(const sc_lv<ADDR_WIDTH>& addr, const char* pin_name) {
    if (addr.is_01()) {
      ...
      if (cur_addr < 0 || cur_addr >= 512) {
#ifdef CCS_SYSC_DEBUG
        std::ostringstream msg;
        msg << "Invalid address '" << cur_addr << "' out of range [0:" << 512-1 << "]";
